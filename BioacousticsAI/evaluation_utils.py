...
def interval_intersection(
    interval1: np.ndarray,
    interval2: np.ndarray,
) -> np.ndarray:
...
def compute_file_precision_recall(
    ...
    file_detections = detections[
        detections.recording_id == os.path.basename(filename)
    ]
    file_annotations = annotations[
        annotations.recording_id == os.path.basename(filename)
    ]

    # Match the bounding boxes by computing the IoU. Discard all matches with
    # IoU less than 0.5
